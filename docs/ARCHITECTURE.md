...
- `jobs/memory-extract/` - Phase 600 LLM-extract over `8.Cortex/Memory/raw/`. `run.sh` med payload-guard + pre-flight + flock + timeout; `extract.py` orkestrerer K5 (aliases.yaml-normalisering, Phase 700) + K6 (Sources-append til `compiled/`, Phase 800). `CONTRACT.md` definerer leveranse-grensesnitt mot obsidian-memory-prosjektet.
...
- `jobs/audit/` - Phase 1000 nattlig audit-pass per `dev-environment/docs/reference/audit-pass-spec.md`. `audit.py` kjører 6 sjekker (dedup / schema / wikilinks / sampling-classification / tags / missing-required-field), aggregerer tier (silent/lav/hoy/kritisk), skriver atomic rapport til `5.Utility/Pipeline/Audit-Reports/YYYY-MM-DD.md` + heartbeat til `~/.audit-state.json`. Watchdog leser heartbeat for 25h-staleness-varsel.
