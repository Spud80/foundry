...
6. **Glob-assert** på `8.Cortex/Memory/raw/<dato>/`. Manifest-handshake (Phase 600): verifisér `_capture-manifest.json` tilstede + alle listed paths lokalt + sha256 matcher. Hvis manifest sier N filer men M ankommet: exit 0 stille (capture sync incomplete; cron retry neste dag).
...
- `cron.d/` - Crontab-fragmenter (én fil per jobb): `auto-update.cron`, `drain-queue.cron`, `memory-extract.cron`, `token-expiry-check.cron`. Konsumeres av `deploy.sh` for atomisk regenerering av claude-bruker-crontab.
- `_shared/` - Felles helpers source'et av flere wrappers/jobber: `notify-core.sh` (queue/drain-implementasjon), `notify.sh` (enqueue + immediate drain), `drain-queue.sh` (cron-drain), `token-expiry-check.sh` (OAuth-token expiry-warning). Underscore-prefiks for at `jobs/`-iterasjon ikke skal traversere.
...
- `jobs/memory-extract/` - Phase E memory-extract. `run.sh` med payload-guard + pre-flight + flock + timeout. `CONTRACT.md` definerer leveranse-grensesnitt mot obsidian-memory-prosjektet (env-vars, exit-codes, manifest-format-referanse). `extract.py` + `system-prompt.md` + `requirements.txt` importeres ved Phase 600 aktivering.
